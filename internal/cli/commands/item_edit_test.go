...
func TestItemEdit_Run_Login_ResolveClient_Applied(t *testing.T) {
	...
	out := withStdoutCapture(t, func() { _ = (itemEditCmd{}).Run(cfg, []string{"--resolve=client", "rec", "login", "alice"}) })
...
func TestItemEdit_Run_FileUpload_And_Sync(t *testing.T) {
	...
	start := time.Now()
	out := withStdoutCapture(t, func() { _ = (itemEditCmd{}).Run(cfg, []string{"note", "file", tmpFile}) })
...
func TestItemEdit_Run_UsageErrors(t *testing.T) {
	// мало аргументов
	if err := (itemEditCmd{}).Run(&config.Config{}, []string{"x"}); err != ErrUsage {
		t.Fatalf("expected ErrUsage, got %v", err)
	}
	// неверный тип
	if err := (itemEditCmd{}).Run(&config.Config{}, []string{"rec", "oops", "val"}); err != ErrUsage {
		t.Fatalf("expected ErrUsage for bad type")
	}
	// неверный resolve
	if err := (itemEditCmd{}).Run(&config.Config{}, []string{"--resolve=bad", "rec", "text", "v"}); err != ErrUsage {
		t.Fatalf("expected ErrUsage for bad resolve")
	}
	// card: не 4 аргумента
	if err := (itemEditCmd{}).Run(&config.Config{}, []string{"rec", "card", "1", "2", "3"}); err != ErrUsage {
